...
5. **ビルド・配布** (2-3日)
   ...
   - [ ] インストーラー生成テスト（`.dmg`, `.msi`, `.AppImage`）（未実施、次のステップ）
   - [ ] コード署名の調査・設定（未実施）
   - [ ] 自動更新機能の検討（`tauri-plugin-updater`）（未実施）
...
- [ ] Windows / macOS / Linux で動作するビルド成果物が生成できる（未実施、次のステップ）
- [ ] 実際のビルドしたバイナリのインストール検証（未実施、次のステップ）

**次のステップ**:
実際のビルドしたバイナリをインストールして動作検証を行う必要があります。具体的には：
