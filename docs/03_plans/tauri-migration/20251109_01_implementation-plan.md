...
1. **Next.js設定の調整** (1-2日)
   - [x] `next.config.ts` の `output: "export"` 設定（完了、2025-11-23）
   - [x] 動的ルートの `generateStaticParams` 実装（完了、2025-11-23）
   - [x] 画像最適化の無効化設定（完了、2025-11-23）

2. **Service Workerの制御** (1-2日)
   - [x] Tauri環境検出ロジック実装（完了、2025-11-23）
   - [x] PWAとTauriの共存テスト（完了、2025-11-23）
   - [x] キャッシュ戦略の調整（完了、2025-11-23）

3. **Tauri設定の完成** (1-2日)
   - [x] `src-tauri/tauri.conf.json` の最終調整（完了、2025-11-23）
   - [x] CSP設定の確認（完了、2025-11-23）
   - [x] OAuth認証Loopback Server方式への移行（完了、2025-11-23）

4. **ビルド・テスト** (2-3日)
   - [x] テスト実装・動作確認（完了、2025-11-23）
   - [ ] 実際のビルドしたバイナリのインストール検証（未実施、次のステップ）
   - [ ] 各OS向けビルドの動作確認（未実施、次のステップ）
...
- [x] Next.jsが静的エクスポートできる（設定完了、2025-11-23）
- [x] Service Worker制御が正しく動作する（完了、2025-11-23）
- [x] OAuth認証がLoopback Server方式で動作する（完了、2025-11-23）
- [x] PWA版とTauri版が共存できる（完了、2025-11-23）
- [ ] 実際のビルドしたバイナリのインストール検証（未実施、次のステップ）

#### 次のステップ

**実際のビルドしたバイナリのインストール検証**:
- macOS向け `.dmg` ファイルのビルドとインストール検証
- Windows向け `.msi` ファイルのビルドとインストール検証
- Linux向け `.AppImage` ファイルのビルドとインストール検証
- インストール後のアプリ起動・動作確認
- OAuth認証フローの実機検証
...
### Phase 6: 静的化・統合
- [x] Next.js静的エクスポート設定（完了、2025-11-23）
- [x] Service Worker制御実装（完了、2025-11-23）
- [x] Tauri設定完成（完了、2025-11-23）
- [x] OAuth認証Loopback Server方式への移行（完了、2025-11-23）
- [x] テスト実装・動作確認（完了、2025-11-23）
- [ ] 実際のビルドしたバイナリのインストール検証（未実施、次のステップ）
