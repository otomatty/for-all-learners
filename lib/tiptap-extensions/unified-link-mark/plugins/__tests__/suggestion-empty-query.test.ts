...
import { createMinimalMockEditor, createMockOptions } from "@/lib/__tests__/helpers";
import { 
...
describe("Suggestion Plugin - Empty Query Behavior", () => {
  ...
  beforeEach(() => {
    ...
    const state = EditorState.create({
      schema: tiptapSchema,
      plugins: [plugin],
      doc: DOMParser.fromSchema(tiptapSchema).parse(document.createElement("div")),
    });
  ...
  describe("TC-001: Empty bracket query behavior", () => {
    it("should show suggestion UI when user types '[' only", () => {
      ...
      const suggestionState = suggestionPluginKey.getState(editorView.state) as UnifiedLinkSuggestionState;
  ...
  describe("TC-005: Suggestion UI state management", () => {
    it("should distinguish between empty query and no results", () => {
      ...
      const emptyState = suggestionPluginKey.getState(editorView.state) as UnifiedLinkSuggestionState;
      ...
      const noResultsState = suggestionPluginKey.getState(editorView.state) as UnifiedLinkSuggestionState;
